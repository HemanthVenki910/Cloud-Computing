...
def new_ride():
    ...
    user_api9_response = requests.get(url = user_api9_url)
    if user_api9_response.status_code == 200:
        if str(creator_username) not in user_api9_response.text:
            return Response(
...
def join_existing_ride(rideId):
    ...
    user_api9_response = requests.get(url = user_api9_url)
    if user_api9_response.status_code == 200:
        if str(username) not in user_api9_response.text:
            return Response(
    ...
    api9_response = requests.post(url = user_api9_url, json =  request_data)
    if api9_response.status_code == 200:
